...
import contacts from "@/views/user/chat/contacts";
...
export default {
  ...
  channels: state => {
    return state.channels.filter(i => {
      return i.name.toLowerCase().includes(state.chatSearchQuery.toLowerCase());
    });
  },
  contacts: state => {
    return state.contacts.filter(i => {
      return i.email
        .toLowerCase()
        .includes(state.chatSearchQuery.toLowerCase());
    });
