...
export default {
  ...
  methods: {
    login() {
      this.$vs.loading({ container: "#page-login", scale: 0.6 });
      this.$socket.connect();
      // Verify if user is activated
      authApi.login({ email: this.email }).then(
        response => {
          if (!response) {
            this.$vs.notify({
              color: "warning",
            ...
            if (response.status === 1) {
              ...
              this.$acl.change(response.role === 1 ? "admin" : "user");
              this.$store.dispatch("auth/loginAttempt", payload);
            } else if (response.status === 2) {
              this.$vs.notify({
                color: "warning",
                title: "Warning",
                text: "This user is locked. Please contact with Admin.",
                iconPack: "feather",
                icon: "icon-alert-triangle"
              });
              this.$vs.loading.close("#page-login > .con-vs-loading");
