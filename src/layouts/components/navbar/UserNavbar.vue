      ...
      <vs-navbar class="vx-navbar navbar-custom" :color="navbarColor" :class="classObj">
        ...
        <vs-dropdown vs-custom-content vs-trigger-click class="cursor-pointer">
          ...
          <vs-dropdown-menu class="notification-dropdown dropdown-custom">
            ...
            >
              <ul class="bordered-items">
                <li
                  ...
                  @click="gotoNotificationLink(ntf.link)"
                >
            ...
            >
              <span @click="$router.push({path: '/user/notification'})">View All Notifications</span>
            </div>
          </vs-dropdown-menu>
        </vs-dropdown>
...
import downloadApi from "@/api/fileUpload";
...
export default {
  ...
  data() {
    return {
      navbarSearchAndPinList: this.$store.state.userNavbarSearchAndPinList,
      searchQuery: "",
  ...
  computed: {
    data() {
      return this.$store.state.userNavbarSearchAndPinList;
    },
    activeUserInfo() {
  ...
  methods: {
    ...
    async getDownloadUrl() {
      ...
      this.downloadUrl = response[0]
        ? `${SERVER_URL}/${response[0].name}`
        : null;
    },
    checkDownload() {
      if (!this.downloadUrl) {
        this.$vs.notify({
          color: "warning",
          title: "Warning",
          text: "There is no file to download."
        });
      }
    },
    getNotifications() {
      const userId = JSON.parse(localStorage.getItem("userInfo")).id;
      this.$store.dispatch("notification/getUserNotificationList", userId);
    },
  ...
  mounted() {
    ...
    this.$store.dispatch("auth/getUserName");
  },
  socket: {
    events: {
      NTF_INVITE_HELPDESK(msg) {
        this.$vs.notify({
          title: "Notification",
          text: msg
        });
      ...
      async NTF_ARRIVE_CHANNEL_MESSAGE(data) {
        console.log(data);
        ...
        if (this.$route.name !== "chat") {
          let ntfData = {
            ...
            type: false,
            link: `${SERVER_URL}/user/chat`,
            userId: this.userId,
            identify: data.channelId
          };
          const notification = await notificationApi.addUserNotification(
            ntfData
          );
          ntfData["id"] = notification._id;
          this.$store.commit(
            "notification/UPDATE_USER_NOTIFICATION_LIST",
            ntfData
          );
          this.$vs.notify({
            title: "Notification",
            text: `New message from ${data.channelName}`,
            iconPack: "feather",
            icon: "icon-message-square"
          });
        }
      },
      async NTF_DIRECT_MESSAGE(data) {
        console.log(data);
        ...
        if (this.$route.name !== "chat") {
          ...
          this.$vs.notify({
            title: "Notification",
            text: `New message from ${data.senderEmail}`,
            iconPack: "feather",
            icon: "icon-message-square"
          });
        }
