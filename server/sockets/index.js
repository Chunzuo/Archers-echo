...
module.exports = io => {
  io.on("connection", socket => {
    ...
    socket.on("REQ_USER_CONNECTED", userId => {
      userSocket.userConnect(socket.id, userId, () => {
    ...
    socket.on("REQ_UPDATE_PROCESS_STATUS", data => {
      deviceController.setActiveProcess(data, err => {
        if (err) {
          console.log(err);
        }
      });
    ...
    socket.on("REQ_PUBLISH_ENROLLMENT", data => {
      enrollmentController.add(data, err => {
        let status = err ? false : true;
        socket.emit("RES_PUBLISH_ENROLLMENT", status);
      });
    });

    /**HelpDeskTicket listeners */
    socket.on("REQ_ADD_HELPDESK", data => {
      helpDeskController.addTicket(data, (err, result) => {
        const response = {
          error: err,
          result: result
        };
        socket.emit("RES_ADD_HELPDESK", response);

        // Send Response to Assigned Members
        const message = `${data.email} invite you to ${data.name}`;
        helpDeskController.getMemerbsSocketInfo(
          data.assignTo,
          message,
          result._id,
          socketIds => {
            for (const socketId of socketIds) {
              console.log("Notification sent to" + socketId);
              io.to(socketId).emit("NTF_INVITE_HELPDESK", message);
            }
          }
        );
      });
    ...
    /**Chat listeners */
    socket.on("REQ_SEND_CHANNEL_MESSAGE", data => {
      chatController.insertChannelMessage(data, io);
    });
    socket.on("REQ_SEND_DIRECT_MESSAGE", data => {
      chatController.addDirectMessage(data, io);
    });
  });
};
