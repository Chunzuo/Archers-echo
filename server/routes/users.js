//Here is where we declare the modules and shit we will need
const express = require("express");

//import the controllers and middleware
const { usersController } = require("../controllers/index");
const { catchErrors } = require("../middleware/error-handler");

//set up the router
const router = express.Router();

//get all users
router.get("/", usersController.list);
router.get("/getOthers/:myId", usersController.getOthers);
router.post("/updateStatus", usersController.updateStatus);
router.post("/updatePassword", usersController.updatePassword);
router.post("/updateSocketId", usersController.updateSocketIdByUserId);
router.post("/passwordReset", usersController.passwordReset);

router.post("/", usersController.addUser);
router.post("/updateProfile", usersController.updateProfile);

//see one boy
router.get("/:id", catchErrors(usersController.show));

router.delete("/", catchErrors(usersController.delete));

//update a boy
router.put("/:id", catchErrors(usersController.update));

module.exports = router;
