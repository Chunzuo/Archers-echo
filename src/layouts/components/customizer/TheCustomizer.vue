      ...
      <div class="h-full">
        ...
        <VuePerfectScrollbar class="scroll-area--customizer pt-4 pb-6" :settings="settings">
          <div class="px-6">
            ...
            <div class="mt-4 flex justify-between">
              <div></div>
              <vs-button @click="updateThemeConfig">Save</vs-button>
            </div>
          </div>
        </VuePerfectScrollbar>
      </div>
...
export default {
  ...
  computed: {
    theme: {
      ...
      set(val) {
        this.$store.commit("themeConfig/UPDATE_THEME", val);
      }
    },
    reduced_sidebar: {
      get() {
    ...
    primaryColor: {
      ...
      set(val) {
        this.$store.commit("themeConfig/UPDATE_PRIMARY_COLOR", val);
      }
    },
  ...
  methods: {
    ...
    async updateThemeConfig() {
      await themeConfigApi.updateSetting(this.$store.state.themeConfig);
      this.$vs.notify({
        title: "Success",
        text: "Success in update theme configuration",
        iconPack: "feather",
        icon: "icon-check",
        color: "success"
      });
